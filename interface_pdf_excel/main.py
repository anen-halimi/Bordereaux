...
def create_fused_results_sheet(extracted_excel_path):
...
def extract_and_order_blocks(pdf_path, output_excel):
    ...
    for i, (img_pil, pdf_page) in enumerate(zip(pages_img, pdf.pages)):
        ...
        for y, typ, content in blocks:
            if typ == 'title':
                ...
                print(f"  ➤ Titre à y={int(y)} : {content}")
...
def create_global_summary(extracted_excel_path):
    ...
    wb.save(extracted_excel_path)
    print(f"✅ Résumé Global finalisé avec colonnes séparées dans : {extracted_excel_path}")
...
from tkinter import filedialog, Tk
import time
def fix_duplicate_realisations_in_excel(file_path):
    from openpyxl import load_workbook
    import re
    ...
    if "Résultat Fusionné" not in wb.sheetnames:
        print(f"❌ Feuille 'Résultat Fusionné' manquante dans {file_path}")
        return
    ...
    for row in ws.iter_rows(min_row=1, max_col=1):  # colonne A
        ...
        if val_final != val:
            ...
            corrected = True
            print(f"🛠️ Corrigé ligne {cell.row} ➤ {val} → {val_final}")

    if corrected:
        wb.save(file_path)
        print(f"✅ Corrections sauvegardées dans : {file_path}")
    else:
        print(f"✅ Aucun doublon 'Nombre de réalisation' à corriger dans : {file_path}")


def check_duplicate_realisations_in_excel(file_path):
    from openpyxl import load_workbook
    import re
    ...
    wb = load_workbook(file_path)
    if "Résultat Fusionné" not in wb.sheetnames:
        print("❌ Feuille 'Résultat Fusionné' non trouvée.")
        return
    ...
    for row in ws.iter_rows(min_row=1, max_col=1):
        ...
        if len(matches) > 1:
            print(f"⚠️ Doublon détecté ligne {cell.row} ➤ {val}")
            found = True

    if not found:
        print("✅ Aucun doublon trouvé dans les titres.")
...
def process_all_pdfs_in_folder(pdf_folder):
    ...
    for pdf_file in os.listdir(pdf_folder):
        if pdf_file.endswith(".pdf"):
            ...
            output_excel = os.path.join(output_dir, pdf_file.replace(".pdf", "_fusion_finale_TOPDOWN_OK.xlsx"))
    ...
    # Fusion de tous les Résumés Globaux
    ...
    if all_data:
        ...
        print("❌ Aucun résumé global trouvé à fusionner.")
...
# Importe ici toutes tes fonctions existantes
# ... (les fonctions extract_and_order_blocks, create_fused_results_sheet, etc. sont inchangées)

# Nouvelle version de traitement d'un seul PDF
def process_single_pdf(pdf_path, output_dir):
    output_excel = os.path.join(output_dir, os.path.basename(pdf_path).replace(".pdf", "_fusion_finale_TOPDOWN_OK.xlsx"))
    print(f"\n\U0001F4C4 Traitement : {os.path.basename(pdf_path)}")
    try:
        extract_and_order_blocks(pdf_path, output_excel)
        create_fused_results_sheet(output_excel)
        create_global_summary(output_excel)
...
# Version modifiée pour traitement parallèle
def process_all_pdfs_in_folder(pdf_folder):
    output_dir = os.path.join(pdf_folder, "outputs")
    os.makedirs(output_dir, exist_ok=True)
    ...
    Parallel(n_jobs=n_cores)(
        delayed(process_single_pdf)(pdf_path, output_dir)
    ...
    # Fusion de tous les Résumés Globaux
    print("\n\U0001F4E6 Fusion des Résumés Globaux...")
    all_data = []
    for file in os.listdir(output_dir):
        if file.endswith("_fusion_finale_TOPDOWN_OK.xlsx"):
            path = os.path.join(output_dir, file)
    ...
    if all_data:
        ...
        final_path = os.path.join(output_dir, "Résumé_Global_Complet.xlsx")
        final_df.to_excel(final_path, index=False)
        print(f"\n\U0001F4CA Fichier résumé final sauvegardé : {final_path}")
    else:
        print("\u274C Aucun résumé global trouvé à fusionner.")
