...
def create_fused_results_sheet(extracted_excel_path, log_func):
...
def extract_and_order_blocks(pdf_path, output_excel, log_func):
...
from tkinter import filedialog, Tk
import time
def fix_duplicate_realisations_in_excel(file_path, log_func):
    from openpyxl import load_workbook
    import re
...
def check_duplicate_realisations_in_excel(file_path, log_func):
    from openpyxl import load_workbook
    import re
...
def process_single_pdf(pdf_path, output_dir, log_func):
...
def process_all_pdfs_in_folder_with_logs_parallel(pdf_folder, log_func=print):
    import pandas as pd
    import multiprocessing
    ...
    log_func(f"🧵 Traitement parallèle avec {n_cores} cœurs (threads)...")

    # Utilisation de ThreadPoolExecutor (et non joblib)
    with ThreadPoolExecutor(max_workers=n_cores) as executor:
        futures = [
            executor.submit(process_single_pdf, pdf_path, output_dir, log_func)
